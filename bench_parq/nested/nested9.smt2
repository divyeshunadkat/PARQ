...
(rule (=> (and (inv2 a i j N) (< j N) (= a0 (store a j (+ 2 (select a j)))) (= j0 (+ j 1))) (inv2 a0 i j0 N)))

(rule (=> (and (inv2 a i j N) (not (< j N)) (= i0 (+ i 1))) (inv1 a i0 N)))

(rule (=> (and (inv1 a i N) (not (< i N))) (inv3 a 0 N)))

(rule (=> (and (inv3 a i N) (< i N) (= a0 (store a i (- N (select a i)))) (= i0 (+ i 1)) ) (inv3 a0 i0 N)))

(rule (=> (and (inv3 a i N) (not (< i N))) (inv4 a 0 N)))

(rule (=> (and (inv4 a i N) (< i N) (= a0 (store a i (- N (select a i)))) (= i0 (+ i 1)) ) (inv4 a0 i0 N)))
